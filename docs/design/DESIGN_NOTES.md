...
# Seventh increment: -H/--host on clean-dokku.sh

`clean-dokku.sh` operates on the local machine by default, exactly as before. `-H hostname` / `--host hostname` instead operates on that host via passwordless ssh -- e.g. `clean-dokku.sh -H dokku-05.cs.ucsb.edu`.

This is almost entirely wiring, not new logic: increment 5 already built `run_dokku` (local, or ssh to `$DOKKU_TARGET_HOST` if set) into every scan/destroy function this script uses, specifically so a *future* script could reuse them against a remote host without a fourth copy. `-H`/`--host` just sets `DOKKU_TARGET_HOST` once at startup instead of per-host-per-iteration the way `clean-all-dokkus.sh` does.

- **`-h` was already `--help`**, in every script in this repo including this one -- proposing `-h <hostname>` (as first suggested) would have collided with that. Asked the user, who wanted *both* `-H`/`--host` (capital H) as the new flag *and* to keep `-h`/`--help` untouched, so `clean-dokku.sh` now parses multiple flags/values in a real `while`/`case` loop instead of the single `case "${1:-}"` it used before (every other script here only ever accepts zero or one bare flag, so this is the first one that needed real multi-arg parsing).
- **The local-`dokku`-required check is skipped when `-H`/`--host` is given** -- mirrors `clean-all-dokkus.sh`, where the control host never needs `dokku` installed locally since every call goes to the remote host via ssh instead.
- **Backtitle names the target** (`... -- dokku-05.cs.ucsb.edu` or `... -- local machine`) so it's never ambiguous which host a session is actually operating on.
- No new error handling was needed for an unreachable target: `run_dokku`'s ssh failure already flows through the same "`scan_unlinked_service`/`scan_dokku_apps` return non-zero -> caller shows a dialog" path built in increment 5.
- Verified end-to-end against dokku-05 (a real, non-dokku-00 host): backtitle and scan correctly targeted it, a real unlinked Postgres database was found, confirmed, and destroyed there (independently verified against dokku-05 directly, and confirmed dokku-00 never had it), and local (no `-H`) operation was regression-tested afterward.

# Eighth increment: shared protected-hosts list; "Destroy Everything" parity for clean-dokku.sh -H

Two related asks: generalize the single hardcoded `PROD_HOSTNUM="00"` exclusion in `clean-all-dokkus.sh` into a real, extensible list; and bring `clean-dokku.sh -H`/`--host` up to parity with `clean-all-dokkus.sh` by offering the same `"[Destroy Everything on <host>]"` entry there too (never for a protected host).

- **`lib/dokku-hosts.sh` gained `DOKKU_PROTECTED_HOSTS`** (one hostname per line, heredoc-assigned, alongside the existing `DOKKU_HOST_FIRST`/`LAST`/`PREFIX`/`SUFFIX` block) and `is_protected_host <hostname>`. Considered converting the *whole* host list to the same one-per-line style for uniformity, but decided against it: the numeric range is simple, typo-proof (hostnames are generated, never hand-typed), and matches the current reality of a clean contiguous `dokku-00`..`dokku-18` range -- there was no concrete problem with it to fix. The actual gap was the protected side, which is inherently an exception list rather than a generated range, and needed to support more than exactly one entry.
- **`clean-all-dokkus.sh`** now calls `is_protected_host "$host"` instead of comparing `$hostnum` to a hardcoded constant -- same behavior, defined in one place.
- **`clean-dokku.sh -H`/`--host`** gets the identical entry when the target isn't protected, built and dispatched the same way as `clean-all-dokkus.sh`'s version but simpler: since there's only one target host for the whole process, `DOKKU_TARGET_HOST` is already set once at startup, so there's no need to set/unset it around the destroy sequence or filter a flat multi-host array afterward -- the local `pg_names`/`mongo_names`/`app_rows`/`app_names` arrays just get reset to empty. Scoped to only appear when `-H`/`--host` is given, per the user's own example invocation -- not offered in local mode, which would need detecting the local machine's own hostname for no clear benefit yet.
- **Found a serious, previously-undetected bug while testing this on dokku-06**: `run_dokku`'s `ssh` call didn't redirect its own stdin. Called from inside a `while read -r x; do ... run_dokku ...; done <<< "$data"` loop -- which both the "destroy everything" sweep *and* `destroy_dokku_app`'s own per-app unlink/destroy loop use whenever there's more than one linked/remaining database -- `ssh` inherits and reads from that loop's stdin (the here-string), silently starving the `while read` of every line after the first. The loop still exits 0, so this fails **silently**: only the first item in any such list is ever processed, the rest are quietly skipped, and nothing indicates a problem. This is why the very first live "Destroy Everything" test (dokku-01, increment 5) looked clean -- it happened to have exactly one leftover item for the sweep to catch, which looks identical whether the loop is correct or silently broken. It only became visible testing dokku-06, which had *five* remaining Postgres databases: the sweep destroyed exactly one and stopped. Fixed with a single-line, single-point change -- `ssh -n` (redirect stdin from `/dev/null`) in `run_dokku` itself -- which is safe for every current caller (none of the dokku commands invoked here need interactive stdin) and fixes every affected call site at once rather than patching each loop individually. Confirmed both the failure and the fix directly with a minimal 3-item loop before re-running the real test. **This bug only affects the ssh (remote-host) path -- local-only operation (increments 1-4) was never affected.**
- Verified end-to-end: the protected-hosts refactor was regression-tested on `clean-all-dokkus.sh` (dokku-00 still correctly excluded); the new entry was confirmed present for dokku-06 and dokku-01, and confirmed *absent* when `-H dokku-00.cs.ucsb.edu` was used; and, after the stdin fix, a full "Destroy Everything" run against dokku-06 (27 apps plus databases) correctly destroyed everything, including the multi-item sweep this time -- independently confirmed dokku-06 reached true zero apps/Postgres/Mongo.
